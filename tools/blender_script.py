"""Blender script to render control images of 3D models."""
"""This script is based on https://github.com/allenai/objaverse-rendering/blob/main/scripts/blender_script.py"""
...
def camera_from_euler(
    ...
    elev, azim: degrees
    """
    z = np.sin(elev * np.pi / 180) * radius
    y = np.cos(elev * np.pi / 180) * np.cos(azim * np.pi / 180) * radius
    x = - np.cos(elev * np.pi / 180) * np.sin(azim * np.pi / 180) * radius
...
def randomize_camera(
    radius_min: float = 1.5*2,
    radius_max: float = 2.2*2,
    maxz: float = 1.6*2,
    minz: float = -0.75*2,
...
def apply_hdri(path_to_image, bg_color, hdri_light_strength, bg_strength):
    ...
    # create the second Background node
...
def get_calibration_matrix_K_from_blender(camd):
    ...
    # focal length, Pytorch3D NDC is sensor (-1, +1)
...
def apply_conditional_material_to_all_objects():
    roughnesses = [0., 1.]
    metallics = [0., 1.]
    
    for i_r in range(len(roughnesses)):
        roughness = roughnesses[i_r]
        for i_m in range(len(metallics)):
            metallic = metallics[i_m]

            material = bpy.data.materials.new(name=f"m{i_m}r{i_r}")
            ...
            principled_bsdf_node.inputs["Metallic"].default_value = metallic
...
def swap_material(i, j):
    for obj in bpy.context.scene.objects:
        if obj.type == "MESH":
            # Swap the original and conditional material
            obj.active_material_index = i
            current_material = obj.active_material
            
            obj.active_material = obj.material_slots[j].material
            obj.active_material_index = j
            obj.active_material = current_material
            
            
            # obj.material_slots[0], obj.material_slots[1] = obj.material_slots[1], obj.material_slots[0]
...
def create_output_nodes():
    ...
    depth_file_output.base_path = "" #set path here if you want to save the depth map in a specific directory
    depth_file_output.file_slots[0].path = "image"  # name of your file
    ...
    # links.new(normalize.outputs[0], color_ramp.inputs[0])
...
def render_object(
    ...
    # Apply conditional material to objects
    apply_conditional_material_to_all_objects()
    ...
    # randomize_lighting()
    hdri_paths = glob.glob(HDRI_PATH)
    hdri_path = random.choice(hdri_paths)
    print(hdri_path)
    apply_hdri(hdri_path, (0.0,0.0,0.0,0.0), hdri_light_strength=1.0, bg_strength=0.0)
    
    depth_output_node, normal_output_node = create_output_nodes()
    ...
    for i in range(num_renders):
        ...
        # camera = _set_camera_at_size(i % 6, scale = 5)
        camera.data.sensor_fit = 'VERTICAL'
        
        depth_output_node.base_path = output_dir
        depth_output_node.file_slots[0].path = f"{i:03d}_depth"  # name of your file
        
        normal_output_node.base_path = output_dir
        normal_output_node.file_slots[0].path = f"{i:03d}_normal"  # name of your file

        # render the image
        render_path = os.path.join(output_dir, f"{i:03d}_rgb.png")
        ...
        # k_matrix = get_calibration_matrix_K_from_blender(camera.data)
        ...
        for i_r in range(2):
            for i_m in range(2):
                swap_material(0, idx)
                # render the image
                render_path = os.path.join(output_dir, f"{i:03d}_m{i_m}r{i_r}.png")
                scene.render.filepath = render_path
                bpy.ops.render.render(write_still=True)
                swap_material(idx, 0)
                idx += 1
        ...
        swap_material(0, 5)
        render_path = os.path.join(output_dir, f"{i:03d}_m5r5.png")
        scene.render.filepath = render_path
        bpy.ops.render.render(write_still=True)
        swap_material(5, 0)
    
    print(metadata_extractor.get_scene_size())
