...
class DeepFloydGuidance(BaseObject):
    ...
    def forward_unet(
        ...
        model_input = torch.cat([latents, latents], dim=1).to(self.weights_dtype)
        return self.unet(
            model_input,
            t.to(self.weights_dtype),
            encoder_hidden_states=encoder_hidden_states.to(self.weights_dtype),
            class_labels=t.to(self.weights_dtype)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        latents = F.interpolate(
            rgb_BCHW, (self.cfg.img_res, self.cfg.img_res), mode="bilinear", align_corners=False
        )
