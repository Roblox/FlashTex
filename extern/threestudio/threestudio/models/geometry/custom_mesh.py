...
class CustomMesh(BaseExplicitGeometry):
    ...
    class Config(BaseExplicitGeometry.Config):
        ...
        shape_init_center: str = "mean"
        shape_init_scaling: str = "max"
    ...
    def configure(self) -> None:
        ...
        if self.cfg.shape_init.startswith("mesh:"):
            ...
            if self.cfg.shape_init_center == "mean":
                # move to center
                centroid = mesh.vertices.mean(0)
                mesh.vertices = mesh.vertices - centroid
            elif self.cfg.shape_init_center == "none":
                # no centering
                pass
            else:
                verts_min = mesh.vertices.min(0)
                verts_max = mesh.vertices.max(0)
                verts_center = verts_min + (verts_max - verts_min) * 0.5
                mesh.vertices = mesh.vertices - verts_center
            ...
            # scaling
            if self.cfg.shape_init_scaling == "max":
            ...
            self.mesh._v_tex, self.mesh._t_tex_idx = v_tex, t_pos_idx
